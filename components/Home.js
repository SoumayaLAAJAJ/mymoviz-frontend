...
function Home() {
  ...
  useEffect(() => {
    fetch('mymoviz-backend-ashy.vercel.app/movies')
    .then(response => response.json())
    .then(data => {setMoviesAPI(data.movies)});
      // const newMovies = []
      // newMovies.map((data, i) => {
      //   return <Movie key={i} title={data.title} overview={data.overview} poster={data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} updateLikedMovies={updateLikedMovies} deleteLikedMovies={deleteLikedMovies}/>
      // });
      // setMoviesAPI([...moviesAPI, newMovies, data.results]);
  ...
  const movies = moviesAPI.map((data, i) => {
    ...
    let tronq;
      if(data.overview.length > 250 ){
        tronq =data.overview.slice(0, 249);
      }else{
        tronq = data.overview;
      }
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={tronq} poster={`https://image.tmdb.org/t/p/w500/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
